...
def analyze_sentiment():
    """Analyzes sentiment and returns only the result."""
    try:
...
def create_journey_endpoint():
    """
    Creates a full, personalized music therapy journey based on the provided sentiment using playlist URIs.
    """
    ...
    try:
        ...
        for stage in journey_plan:
            playlist_uri = stage['playlist_uri']
            if playlist_uri:
                ...
                return jsonify({'error': f'No playlist URI available for {stage["label"]}.'}), 500
...
def play_song():
    ...
    try:
        ...
        print(f"Attempting to play playlist {playlist_uri} on device {device_id}")
